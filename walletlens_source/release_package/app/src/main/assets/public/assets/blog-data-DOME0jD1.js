const u=[],d=new Set("the a an and or of to in for your you how what is are be on with that this it as at by from across into not but new — your guide complete".split(" "));function s(a){return new Set(`${a.title} ${a.summary}`.toLowerCase().replace(/[^a-z0-9 ]/g," ").split(/\s+/).filter(t=>t.length>3&&!d.has(t)))}function p(a,t=3){const i=r.findIndex(e=>e.slug===a);if(i<0)return r.slice(0,t);const l=s(r[i]);return r.map((e,o)=>{if(o===i)return null;const c=s(e);let n=0;for(const h of c)l.has(h)&&n++;return{p:e,overlap:n,i:o}}).filter(Boolean).sort((e,o)=>o.overlap-e.overlap||e.i-o.i).slice(0,t).map(e=>e.p)}const y=[{slug:"track-real-estate-alongside-crypto-and-stocks",title:"How to Track Real Estate Alongside Crypto and Stocks",date:"July 2026",readTime:"7 min read",summary:"Learn how to include property in your net-worth picture and track real estate alongside crypto and stocks in one unified portfolio view.",content:`
