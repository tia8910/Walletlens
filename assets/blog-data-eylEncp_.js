const u=[],d=new Set("the a an and or of to in for your you how what is are be on with that this it as at by from across into not but new — your guide complete".split(" "));function n(o){return new Set(`${o.title} ${o.summary}`.toLowerCase().replace(/[^a-z0-9 ]/g," ").split(/\s+/).filter(t=>t.length>3&&!d.has(t)))}function p(o,t=3){const i=r.findIndex(e=>e.slug===o);if(i<0)return r.slice(0,t);const l=n(r[i]);return r.map((e,a)=>{if(a===i)return null;const c=n(e);let s=0;for(const h of c)l.has(h)&&s++;return{p:e,overlap:s,i:a}}).filter(Boolean).sort((e,a)=>a.overlap-e.overlap||e.i-a.i).slice(0,t).map(e=>e.p)}const y=[{slug:"how-to-track-average-cost-basis-across-multiple-buys",title:"How to Track Average Cost Basis Across Multiple Buys",date:"July 2026",readTime:"7 min read",summary:"Learn how average cost basis works when you buy an asset multiple times, why it matters for P&L, and how to track it automatically for free.",content:`
