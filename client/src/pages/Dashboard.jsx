...
// A single holdings-list row. Memoized so that unrelated Dashboard state
// (other panels, filters, the 60s price poll when this holding's own price
// didn't change) doesn't re-render every row in the list — only the row(s)
// whose own props actually changed.
const HoldingRow = memo(function HoldingRow({
  h, isDupTicker, isSelected, isDimmed, actionsOpen,
  pricesFailed, isDemo, showBreakEven, cv, navigate,
  setSelectedAssets, setExpandedActions, showLp,
}) {
  const { t } = useLanguage()
  ...
  return (
    <li className={`dvx-holding holo-card-v2${isSelected ? ' selected' : ''}`}
...
export default function Dashboard() {
  ...
  const tabs = [
    { id: 'overview',   label: t('dashboard'),   icon: Ico.overview,  color: '#10b981' },
    { id: 'watchlist',  label: t('watchlist'),   icon: Ico.watchlist, color: '#38bdf8' },
    { id: 'tools',      label: t('analysis'),    icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 3v16a2 2 0 0 0 2 2h16"/><path d="M7 14.5l3.5-4 3 2.5L21 7"/><circle cx="21" cy="7" r="1.5" fill="currentColor" stroke="none"/></svg>, color: '#a78bfa' },
    { id: 'alerts',     label: t('alerts'),      icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>, color: '#fbbf24' },
    { id: 'targets',    label: t('targets'),  icon: Ico.target,    color: '#fb7185' },
    { id: 'manage',     label: t('backup'),      icon: Ico.wallet,    color: '#2dd4bf' },
  ]
                              ...
                              {(() => {
                                ...
                                return grouped[cat].map(h => (
                                  <HoldingRow
                                    ...
                                    isDimmed={selectedAssets.size > 0 && !selectedAssets.has(h.coin_id)}
                                    actionsOpen={expandedActions.has(h.coin_id)}
