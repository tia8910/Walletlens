...
const WISDOM_TIPS = [
  ...
  '⚙️ Set price alerts and stop-losses — automating risk management removes emotion from the equation',
...
function buildTips(holdings, totalValue, totalPnLPct, sentimentScore) {
  ...
  if (isBearish) {
    ...
    tips.push('🏔️ Bear markets are where generational wealth is built — every dip is a potential accumulation opportunity for the patient')
  }
  ...
  if (topPct > 60) tips.push(`⚖️ ${topHolding.coin_symbol?.toUpperCase()} makes up ${topPct.toFixed(0)}% of your portfolio — heavy concentration amplifies both gains and losses`)
  else if (topPct > 40) tips.push(`⚖️ ${topHolding.coin_symbol?.toUpperCase()} is your largest position at ${topPct.toFixed(0)}% — consider whether that exposure matches your risk tolerance`)
  ...
  if (losers.length) {
    const l = losers[0]
    tips.push(`⚠️ ${l.coin_symbol?.toUpperCase()} is down ${Math.abs(l.pnlPct).toFixed(1)}% — review your thesis or set a stop-loss to protect remaining capital`)
  ...
  if (totalPnLPct != null) {
    if (totalPnLPct > 50) tips.push(`🏆 Portfolio up ${totalPnLPct.toFixed(1)}% overall — outstanding! Review profit targets and consider rebalancing to protect those gains`)
      ...
      tips.push(`❤️ Portfolio down ${Math.abs(totalPnLPct).toFixed(1)}% — that's painful, but drawdowns are part of every investor's journey. Focus on what you can control: risk size and your exit plan`)
