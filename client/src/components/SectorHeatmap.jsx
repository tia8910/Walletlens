...
export default function SectorHeatmap() {
  ...
  useEffect(() => {
    if (!open) return
    const now = Date.now()
    if (_cache && now - _cacheTime < 10 * 60 * 1000) {
      setTiles(_cache)
      return
    }
    setLoading(true)
    ...
    <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '1rem', overflow: 'hidden', marginTop: '1rem' }}>
      ...
      {open && (
        <div style={{ padding: '0 1.1rem 1.1rem' }}>
          ...
          {error && <div style={{ color: '#f87171', fontSize: '0.82rem', padding: '0.5rem 0' }}>{error}</div>}
          {tiles && (
            <>
                ...
                {tiles.map(({ sector, avg, top }) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {top && (
                        <div style={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.4)', marginTop: '0.25rem', textTransform: 'uppercase', letterSpacing: '0.03em' }}>
                          ↑ {top.symbol}
                        </div>
                      )}
                    </div>
