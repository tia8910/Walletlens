...
//
// Settings.jsx once shipped a call to a settings helper with no import for
// it.
...
const WATCHED = [
  'screenEffectsRuntime.js',
  'screenEffects.js',
  'screenEffectsAudio.js',
  'reviewPrompt.js',
  'nativeBridge.js',
  'nativeWidgets.js',
]
...
describe('every used helper is imported', () => {
  ...
  it('knows what it is looking for', () => {
    ...
    expect(watched.has('effectSettings')).toBe(true)
    expect(watched.has('observe')).toBe(true)
  })
  ...
  it('finds no helper called without an import', () => {
    ...
    for (const full of sourceFiles(join(SRC, 'pages')).concat(sourceFiles(join(SRC, 'components')))) {
      ...
      for (const [name, from] of watched) {
        ...
        // `audio.play()` reads as a call to an exported play().
